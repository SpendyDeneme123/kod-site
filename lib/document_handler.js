...
var DocumentHandler = function(options) {
	...
	if (this.createKey !== '') {
		logger.info("Oluşturulan Anahtar:", this.createKey);
	}
};
...
DocumentHandler.prototype.handleGet = function(key, res) {
	this.store.get(key, function(ret) {
		if (ret) {
			logger.verbose('Açilan kod:', key);
			res.writeHead(200, {'content-type': 'application/json'});
			res.end(JSON.stringify({key: key, data: ret.replace(/\t/g, '    ')}));
		} else {
			logger.verbose('Kod bulunumadi:', key);
			res.writeHead(404, {'content-type': 'application/json'});
			res.end(JSON.stringify({message: 'Kod bulunumaadi'}));
		}
	});
};
